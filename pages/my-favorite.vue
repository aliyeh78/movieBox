<template>
  <div class="max-w-7xl mx-auto py-10 px-4">
    ...
    <!-- SSR Guard / Skeleton -->
    <div v-if="!mounted" class="space-y-6 animate-pulse">
      <div class="h-8 w-48 bg-neutral-700 rounded"></div>
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-7">
        <div v-for="n in 10" :key="n" class="aspect-[2/3] bg-neutral-800 rounded-2xl" />
      </div>
    </div>
    ...
    <template v-else>
      ...
      >
        ...
        <NuxtLink
          to="/"
          class="mt-6 inline-flex items-center gap-2 bg-accent px-5 py-2 rounded-lg text-white hover:scale-105 transition"
        >
          Browse Movies
        </NuxtLink>
      </div>
      ...
      >
        ...
        >
          <NuxtLink :to="getLink(item)">
            ...
            <!-- Poster ratio container -->
            <div class="w-full aspect-[2/3] bg-neutral-800">
              <img
            ...
            <!-- Gradient overlay -->
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-80 group-hover:opacity-100 transition" />

            <!-- Rating -->
            <div v-if="item.vote_average" class="absolute top-2 left-2 z-20 opacity-0 group-hover:opacity-100">
              <div class="relative w-8 h-8 bg-black/70 rounded-full flex items-center justify-center">
                <svg class="w-8 h-8 -rotate-90" viewBox="0 0 36 36">
                  ...
                  <circle
                    ...
                    :stroke-dashoffset="getRatingPercent(item.vote_average)"
            ...
            <button
              ...
              class="absolute top-2 right-2 w-9 h-9 rounded-full bg-black/60 backdrop-blur flex items-center justify-center transition-transform active:scale-75 hover:scale-110"
            >
...
const favoriteMap = computed(() => {
  return new Set(fav.favorites.value.map(f => `${f.id}-${f.media_type}`));
});
...
function isFavorite(item: FavoriteItem) {
  return favoriteMap.value.has(`${item.id}-${item.media_type}`);
}
...
function getRatingPercent(vote:number){
  return 100 - vote * 10
}
</script>
